...
#include "KineticsMap_Solid_CHEMKIN.h"
				...
				// Stoichiometry
				{
					...
					changeOfMoles_ = stoichiometry_->changeOfMoles();
					{
						...
						for(unsigned int k=1;k<=number_of_thermodynamic_reversible_reactions_;k++)
							tmp[indices_of_thermodynamic_reversible_reactions_[k]-1] = true;
...
//			reverseReactionRates_[j] *= kArrhenius_reversible_[k]/kArrheniusModified_[j];
	...
	{
		ErrorMessage("KineticsMap_Solid_CHEMKIN::WriteKineticDatas", "Function not yet available!");
	}
	...
	{
		ErrorMessage("KineticsMap_Solid_CHEMKIN::FittedReverseKineticConstants", "Function not yet available!");
	}
