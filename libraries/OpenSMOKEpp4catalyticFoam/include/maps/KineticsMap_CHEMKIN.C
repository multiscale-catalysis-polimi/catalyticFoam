...
#include "KineticsMap_CHEMKIN.h"
#include "kernel/kinetics/ExtendedFallOff.h"

namespace OpenSMOKE
{
	KineticsMap_CHEMKIN::KineticsMap_CHEMKIN(ThermodynamicsMap_CHEMKIN& thermo, const unsigned int nSpecies) :
	...
	KineticsMap_CHEMKIN::KineticsMap_CHEMKIN(ThermodynamicsMap_CHEMKIN& thermo, rapidxml::xml_document<>& doc) :
	thermodynamics_(thermo)
	{           
        this->verbose_output_ = true;
        this->isJacobianSparsityMapAvailable_ = false;
	...
	KineticsMap_CHEMKIN::KineticsMap_CHEMKIN(ThermodynamicsMap_CHEMKIN& thermo, rapidxml::xml_document<>& doc, bool verbose) :
	thermodynamics_(thermo)
	{         
	...
	void KineticsMap_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)
	{
		    ...
		    std::cout << " * Reading kinetic parameters of reactions..." << std::endl;	
		{
			...
			if (number_of_falloff_reactions_ != 0)
			{
				...
				// Parameters
				{
					falloff_reaction_type_.resize(number_of_falloff_reactions_);
					...
					for (rapidxml::xml_node<> *current_node = parameters_node->first_node("reaction"); current_node; current_node = current_node->next_sibling())
					{
						...
						if (dummy == "lindemann")
						{
							falloff_reaction_type_[i-1] = PhysicalConstants::REACTION_LINDEMANN_FALLOFF;
						}
						if (dummy == "troe")
						{
							falloff_reaction_type_[i-1] = PhysicalConstants::REACTION_TROE_FALLOFF;
						...
						{
							falloff_reaction_type_[i-1] = PhysicalConstants::REACTION_SRI_FALLOFF;
			...
			if (number_of_cabr_reactions_ != 0)
			{
				...
				// Parameters
				{
					cabr_reaction_type_.resize(number_of_cabr_reactions_);
					...
					for (rapidxml::xml_node<> *current_node = parameters_node->first_node("reaction"); current_node; current_node = current_node->next_sibling())
					{
						...
						if (dummy == "lindemann")
						{
							cabr_reaction_type_[i-1] = PhysicalConstants::REACTION_LINDEMANN_CABR;
						}
						if (dummy == "troe")
						{
							cabr_reaction_type_[i-1] = PhysicalConstants::REACTION_TROE_CABR;
						...
						{
							cabr_reaction_type_[i-1] = PhysicalConstants::REACTION_SRI_CABR;
		...
		// Stoichiometry
		{
			...
			{
				std::vector<bool> tmp(this->number_of_reactions_);
			...
			{
				type_of_reaction_.resize(this->number_of_reactions_);
				ChangeDimensions(this->number_of_reactions_, &local_family_index_, true);

				std::fill(type_of_reaction_.begin(), type_of_reaction_.end(), PhysicalConstants::REACTION_SIMPLE);

				for(unsigned int k=1;k<=number_of_thirdbody_reactions_;k++)
				{
					type_of_reaction_[indices_of_thirdbody_reactions_[k]-1] = PhysicalConstants::REACTION_THIRDBODY;
					local_family_index_[indices_of_thirdbody_reactions_[k]] = k;
				}
				for(unsigned int k=1;k<=number_of_chebyshev_reactions_;k++)
				{
					type_of_reaction_[indices_of_chebyshev_reactions_[k]-1] = PhysicalConstants::REACTION_CHEBYSHEV;
					local_family_index_[indices_of_chebyshev_reactions_[k]] = k;
				}

				for(unsigned int k=1;k<=number_of_falloff_lindemann_reactions_;k++)
					type_of_reaction_[indices_of_falloff_lindemann_reactions_[k]-1] = PhysicalConstants::REACTION_LINDEMANN_FALLOFF;	
				for(unsigned int k=1;k<=number_of_falloff_troe_reactions_;k++)
					type_of_reaction_[indices_of_falloff_troe_reactions_[k]-1] = PhysicalConstants::REACTION_TROE_FALLOFF;
				for(unsigned int k=1;k<=number_of_falloff_sri_reactions_;k++)
					type_of_reaction_[indices_of_falloff_sri_reactions_[k]-1] = PhysicalConstants::REACTION_SRI_FALLOFF;
				...
				for(unsigned int k=1;k<=number_of_cabr_lindemann_reactions_;k++)
					type_of_reaction_[indices_of_cabr_lindemann_reactions_[k]-1] = PhysicalConstants::REACTION_LINDEMANN_CABR;
				for(unsigned int k=1;k<=number_of_cabr_troe_reactions_;k++)
					type_of_reaction_[indices_of_cabr_troe_reactions_[k]-1] = PhysicalConstants::REACTION_TROE_CABR;
				for(unsigned int k=1;k<=number_of_cabr_sri_reactions_;k++)
					type_of_reaction_[indices_of_cabr_sri_reactions_[k]-1] = PhysicalConstants::REACTION_SRI_CABR;
	...
	void KineticsMap_CHEMKIN::KineticConstants()
	{
		...
		if (arrhenius_kinetic_constants_must_be_recalculated_ == true)
		{
			...
			if (number_of_falloff_reactions_ != 0)
			{
				...
				for(unsigned int k=1;k<=number_of_falloff_reactions_;k++)
				{
					switch(falloff_reaction_type_[k-1])
					{
			...
			if (number_of_cabr_reactions_ != 0)
			{
				...
				for(unsigned int k=1;k<=number_of_cabr_reactions_;k++)
				{
					switch(cabr_reaction_type_[k-1])
					{
	...
	double KineticsMap_CHEMKIN::FallOffReactionsCorrection(const unsigned int local_k, const double cTot, const OpenSMOKEVectorDouble& c)
	{
		...
		double wF = 1.;
		switch (falloff_reaction_type_[local_k-1])
		{
			case PhysicalConstants::REACTION_TROE_FALLOFF:
	...
	void KineticsMap_CHEMKIN::DerivativesOfReactionRatesWithRespectToKineticParameters(const PhysicalConstants::sensitivity_type type, unsigned int jReaction, const OpenSMOKEVectorDouble& c, double& parameter)
	{
		...
		else if (type == PhysicalConstants::SENSITIVITY_FREQUENCY_FACTOR)
		{
			if (jReaction <= this->number_of_reactions_)
			{
				...
				if (type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_LINDEMANN_FALLOFF || 
					type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_TROE_FALLOFF ||
					type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_SRI_FALLOFF)
				{
				...
				else if (type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_LINDEMANN_CABR || 
						 type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_TROE_CABR ||
						 type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_SRI_CABR)
				{
				...
				else if (type_of_reaction_[jReaction-1] == PhysicalConstants::REACTION_CHEBYSHEV)
				{
					parameter = kArrheniusModified_[jReaction];
					kArrheniusModified_[jReaction] = 1.;
				}
...
//		}
	...
	{			
		...
		{
			ChangeDimensions(5, &temperatures, true);
			temperatures[1] = 300.;
			temperatures[2] = 1000.;
			temperatures[3] = 1500.;
			temperatures[4] = 2000.;
		...
		{
			ChangeDimensions(list_of_temperatures.size(), &temperatures, true);
	...
	void KineticsMap_CHEMKIN::WriteKineticData(std::ostream& fOut, const unsigned int k, const double T, const double P_Pa, OpenSMOKEVectorDouble& c)
	{
		...
		if (type_of_reaction_[k-1] == PhysicalConstants::REACTION_LINDEMANN_FALLOFF ||
			type_of_reaction_[k-1] == PhysicalConstants::REACTION_TROE_FALLOFF ||
			type_of_reaction_[k-1] == PhysicalConstants::REACTION_SRI_FALLOFF)
		{
