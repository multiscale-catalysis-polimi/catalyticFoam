...
#include "KineticsMap_Surface_CHEMKIN.h"
...
//			reverseReactionRates_[j] *= kArrhenius_reversible_[k]/kArrheniusModified_[j];
	...
	{
		ErrorMessage("KineticsMap_Surface_CHEMKIN::WriteKineticDatas", "Function not yet available!");
	}
	...
	{
		ErrorMessage("KineticsMap_Surface_CHEMKIN::FittedReverseKineticConstants", "Function not yet available!");
	}
	...
	*/
	...
	void UBIQEP_SubMechanism::ReadFromXMLFile(rapidxml::xml_node<> *xml_node, const OpenSMOKE::OpenSMOKEVectorDouble& MW)
	{
		...
		std::cout << "Reading reaction parameters (direct reactions)" << std::endl;
		{
			...
			ubiqep_type_direct_.resize(half_number_of_ubiqep_reactions_);
			...
			for(unsigned int k=1;k<=half_number_of_ubiqep_reactions_;k++)
			{
				...
				fInput >> dummy;
				ubiqep_type_direct_[k-1] = static_cast<PhysicalConstants::UBIQEP_TYPE>(boost::lexical_cast<unsigned int>(dummy));

				if (ubiqep_type_direct_[k-1] == PhysicalConstants::UBIQEP_TYPE_ADSORPTION)
				{
					fInput >> dummy;
		...
		std::cout << "Reading reaction parameters (reverse reactions)" << std::endl;
		{
			...
			ubiqep_type_reverse_.resize(half_number_of_ubiqep_reactions_);
			...
			for(unsigned int k=1;k<=half_number_of_ubiqep_reactions_;k++)
			{
				...
				fInput >> dummy;    // 
				ubiqep_type_reverse_[k-1] = static_cast<PhysicalConstants::UBIQEP_TYPE>(boost::lexical_cast<unsigned int>(dummy));
			}
		}
	}
	...
	void UBIQEP_SubMechanism::ForwardKineticConstants(const double T, const double total_site_density, OpenSMOKE::OpenSMOKEVectorDouble& kForward)
	{
		...
		for (unsigned int k=1;k<=half_number_of_ubiqep_reactions_;k++)
		{
			...
			if (ubiqep_type_reverse_[k-1] == PhysicalConstants::UBIQEP_TYPE_ADSORPTION)
