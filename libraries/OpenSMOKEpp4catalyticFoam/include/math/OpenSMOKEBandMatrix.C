...
namespace OpenSMOKE
{
	...
	template<typename T>
	void OpenSMOKEBandMatrix<T>::Scale(const T c_differential, const T c_algebraic, const int *index)
	{
		int width = mu + ml + 1;
	...
	template<>
	void OpenSMOKEBandMatrix<float>::CopyTo(OpenSMOKEBandMatrix<float>* B)
	{
		#if __APPLE__
			...
			bandCopy(cols, B->cols, M, s_mu, B->s_mu, mu, ml);
				...
				cblas_scopy(ldata, data, 1, B->data, 1);
	...
	template<>
	void OpenSMOKEBandMatrix<double>::CopyTo(OpenSMOKEBandMatrix<double>* B)
	{
		#if __APPLE__
	...
	template<typename T>
	void OpenSMOKEBandMatrix<T>::Scale(const T c)
	{
		#if __APPLE__
	...
	/*template<>
	void OpenSMOKEBandMatrix<float>::Scale(const float c)
	{
		#if __APPLE__

			cblas_sscal(ldata, c, data, 1);

		#elif (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)

			const int one = 1;
			const int lmat = ldata;
			sscal(&lmat, &c, data, &one);
	...
	template<>
	void OpenSMOKEBandMatrix<double>::Scale(const double c)
	{
		#if __APPLE__
	...
	template<>
	int OpenSMOKEBandMatrix<float>::Factorize()
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)

		const int ier = LAPACKE_sgbtrf(CblasColMajor, M, M, ml, mu, data, ldim, p);
	...
	template<>
	int OpenSMOKEBandMatrix<double>::Factorize()
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)
	...
	template<>
	void OpenSMOKEBandMatrix<float>::Solve(float *b)
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)

			const int one = 1;
			const int ier = LAPACKE_sgbtrs(CblasColMajor, 'N', M, ml, mu, one, data, ldim, p, b, M);
	...
	template<>
	void OpenSMOKEBandMatrix<double>::Solve(double *b)
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)
	...
	template<>
	void OpenSMOKEBandMatrix<float>::Solve(const int nrhs, float *b)
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)

		const int ier = LAPACKE_sgbtrs(CblasColMajor, 'N', M, ml, mu, nrhs, data, ldim, p, b, M);
	...
	template<>
	void OpenSMOKEBandMatrix<double>::Solve(const int nrhs, double *b)
	{
		#if (OPENSMOKE_USE_MKL == 1 || OPENSMOKE_USE_OPENBLAS == 1)
	...
	template class OpenSMOKEBandMatrix<float>;
	template class OpenSMOKEBandMatrix<double>;
}
