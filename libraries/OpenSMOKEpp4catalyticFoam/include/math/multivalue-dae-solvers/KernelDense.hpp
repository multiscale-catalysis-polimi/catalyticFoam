...
namespace DaeSMOKE
{
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::BuildAndFactorizeMatrixG(const double hr0, const double rr0)
	{
		G_ = J_;
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::SolveLinearSystem(Eigen::VectorXd& db)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();
