	...
	*/
	...
	class OpenSMOKELoad : public OpenSMOKEBaseClass
	{
		...
		public:
			...
			template<typename T_>
			inline OpenSMOKELoad &operator >> (OpenSMOKEVector<T_> &v);

			template<typename T_>
			inline OpenSMOKELoad &operator >> (OpenSMOKEMatrix<T_> &A);
	
			//template<typename T_>
			//void readVectorFromBinaryFile(std::ifstream &fInput, int N, OpenSMOKEVector<T_> &v);
