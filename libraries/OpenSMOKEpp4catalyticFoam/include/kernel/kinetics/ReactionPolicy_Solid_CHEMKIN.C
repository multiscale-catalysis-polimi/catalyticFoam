...
#include "ReactionPolicy_Solid_CHEMKIN.h"
	...
	double ReactionPolicy_Solid_CHEMKIN::Arev_conversion() const
    {
        ...
        if(	tag_reaction_ == PhysicalConstants::REACTION_SURFACE_SIMPLE )
        {
            if(iExplicitlyReversible_ == true)
                ...
                OpenSMOKE::ErrorMessage("double Arev_conversion() const", "The reaction is not reversible!");
        }
        else
        {
			OpenSMOKE::ErrorMessage(	"double Arev_conversion() const", "Conversion of A reversible allowed only for "
