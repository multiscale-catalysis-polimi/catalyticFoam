...
#ifndef DaeMultiValueSolver_H
#define DaeMultiValueSolver_H
...
namespace DaeSMOKE
{
	...
	// Static variables
	extern bool stopDaeIntegration = false;

	//!  A class to solve DAE systems using a multivalue method
	/*!
	The purpose of this class is the solution of a DAE system using a proper multivalue method.
	...
	*/
	...
	public:
		...
		*@param equation_type algebraic vs differential equations
		*/
		void SetInitialConditions(const double t0, const Eigen::VectorXd& y0, const std::vector<OpenSMOKE::EquationType>& equation_type);

		/**
		*@brief Solve the DAE system
		*@param tOut the final value of the independent variable
		*/
		DaeStatus Solve(const double tOut);

		/**
		*@brief Summary
		*@param out output stream
		*/
		void DaeSummary(std::ostream& out);
	...
	private:
		...
		*/
		...
		/**
		*@brief Rescale the tolerances
		*/
		void RescaleTolerances();
		...
		*@brief Calculates the initial step size (estimation, which is rapidly adapted after some steps)
		*/
		void CalculatesOneOverEpsilonSystem(const Eigen::VectorXd& w);
		...
		*/
		...
		/**
		*@brief Main function: solves the requested DAE system
		...
		*/
		...
		/**
		*@brief Function to parse the status of the Dae solver
		*/
		void ParseDaeStatus();
	...
	private:
		...
		Eigen::VectorXd one_over_epsilon_system_;	//!< error vector: one_over_epsilon = (tolAbs + tolRel*abs(y))^(-1)
		...
		DaeErrorStatus errorStatus_;		//!< flag to recognize if the error was ok or too large
