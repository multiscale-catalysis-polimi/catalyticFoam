...
namespace DaeSMOKE
{
	...
	int Solve_Band_BzzDae(Object* object, BzzDaeSparseObject& dae_object, const DaeSMOKE::DaeSolver_Parameters& parameters, const double t0, const double tEnd)
	{
		std::cout << "Band DAE solution (BzzDae)..." << std::endl;
		...
		BzzVectorInt inDerAlg(neq);
		{
			...
			for (unsigned int i = 1; i <= object->NumberOfEquations(); i++)
				inDerAlg[i] = int(temp[i]);
		}

		System dae_system;
		dae_system.assign(object);
		dae_object(yInitial, t0, inDerAlg, &dae_system, object->LowerBand(), object->UpperBand());
