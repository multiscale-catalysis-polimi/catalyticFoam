...
namespace DaeSMOKE
{
	...
	*/
	...
	class KernelSparse : public DAESystemObject
	{
	public:
		...
		*/
		void SetSparsityPattern(const unsigned int neq, const std::vector<unsigned int>& i, const std::vector<unsigned int>& j, const bool index_zero);
	...
	protected:
		...
		*/
		bool BuildAndFactorizeMatrixG(const double hr0, const double rr0);
		...
		*/
		bool SolveLinearSystem(Eigen::VectorXd& db);
	...
	private:
		...
		Eigen::VectorXd	y_plus_;				//!< auxiliary vector (dimension equal to the number of equations)
