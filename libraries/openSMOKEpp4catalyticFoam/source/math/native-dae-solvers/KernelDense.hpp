...
namespace DaeSMOKE
{
	...
	KernelDense<DAESystemObject>::KernelDense()
	{
	}

	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::ResetKernel()
	{
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::MemoryAllocationKernel()
	{
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::SetLinearAlgebraSolver(const std::string linear_algebra_solver)
	{
		if (linear_algebra_solver == "Eigen")
		{
			...
			OpenSMOKE::ErrorMessage("KernelDense<DAESystemObject>", "Requested linear algebra is not supported!");
	}

	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::SetFullPivoting(const bool flag)
	{
		full_pivoting_ = flag;
	}

	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::JacobianTimesVector(const Eigen::VectorXd& v_in, Eigen::VectorXd* v_out)
	{
		*v_out = J_*v_in;
	}

	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::UserDefinedJacobian(const Eigen::VectorXd& y, const double t)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();

		// TODO
		// this->Jacobian(y, t, J_);
		OpenSMOKE::ErrorMessage("KernelDense<DAESystemObject>", "User defined Jacobian is still not supported!");
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::NumericalJacobian(Eigen::VectorXd& y, const double t, const Eigen::VectorXd& f, const double h, const Eigen::VectorXd& e,
	...
	template <typename DAESystemObject>
	bool KernelDense<DAESystemObject>::BuildAndFactorizeMatrixG(const double hr0, const double rr0)
	{
		G_ = J_;
	...
	template <typename DAESystemObject>
	bool KernelDense<DAESystemObject>::SolveLinearSystem(Eigen::VectorXd& db)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();
	...
	template <typename DAESystemObject>
	void KernelDense<DAESystemObject>::DaeSolverKernelSummary(std::ostream& out)
	{
		...
		out << std::endl;
		out << "Data for the Dense DAE solver Kernel" << std::endl;
		out << "---------------------------------------------------------------------------------------------------------" << std::endl;
