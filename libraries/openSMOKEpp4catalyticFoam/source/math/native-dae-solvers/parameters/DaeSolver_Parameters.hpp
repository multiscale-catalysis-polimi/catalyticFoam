...
#include "DaeSolver_Parameters_Grammar.h"

namespace DaeSMOKE
{
	DaeSolver_Parameters::DaeSolver_Parameters()
	{
		// Default values
		type_ = DAE_INTEGRATOR_OPENSMOKEPP;
...
//      #endif
		...
		scalar_relative_tolerance_ = true;
		scalar_absolute_tolerance_ = true;
		relative_tolerance_ = 100.*OpenSMOKE::MachEpsFloat();
		absolute_tolerance_ = 1.e-10;
		
		// Default values are adopted for initial and maximuim steps
		minimum_step_ = -1.;
		maximum_step_ = -1.;
		initial_step_ = -1.;
	...
	void DaeSolver_Parameters::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
		DaeSolver_Parameters_Grammar grammar;
		...
		if (dictionary.CheckOption("@DaeSolver") == true)
		{
			...
			else if (name == "BzzDae")
			{
					...
					OpenSMOKE::FatalErrorMessage("OpenSMOKE++ was built without the BzzMath support. Please select a different DAE solver");
				#endif
				type_ = DAE_INTEGRATOR_BZZDAE;
				jacobian_structure_ = OpenSMOKE::JACOBIAN_STRUCTURE_BAND;

				// If the BzzDae integrator is selected, the user can choose the Jacobian structure treatment
				if (dictionary.CheckOption("@Jacobian") == true)
				{
					...
					if (name == "Band")
						jacobian_structure_ = OpenSMOKE::JACOBIAN_STRUCTURE_BAND;
					else if (name == "TridiagonalBlock")
						jacobian_structure_ = OpenSMOKE::JACOBIAN_STRUCTURE_TRIDIAGONAL_BLOCK;
					else OpenSMOKE::FatalErrorMessage("Unknown @Jacobian. Available options: Band (default) | TridiagonalBlock");
				}
			}
			else if (name == "Ida")
			{
				#if OPENSMOKE_USE_SUNDIALS == 0
					OpenSMOKE::FatalErrorMessage("OpenSMOKE++ was built without the Sundials (IDA) support. Please select a different DAE solver");
				#endif
			...
			else if (name == "DASPK")
			{
					...
					OpenSMOKE::FatalErrorMessage("OpenSMOKE++ was built without the DASPK support. Please select a different DAE solver");
				#endif
				type_ = DAE_INTEGRATOR_DASPK;
			}

			else OpenSMOKE::FatalErrorMessage("Unknown DAE Solver: " + name);
		}

		// If the OpenSMOKE++ integrator is selected, the user can choose sparse linear algebra
		if (type_ == DAE_INTEGRATOR_OPENSMOKEPP)
		{
			if (dictionary.CheckOption("@Jacobian") == true)
			{
				std::string name;
				...
				{
					sparse_linear_algebra_ = true;
					jacobian_structure_ = OpenSMOKE::JACOBIAN_STRUCTURE_SPARSE;
				}
