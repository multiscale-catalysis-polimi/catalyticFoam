...
namespace DaeSMOKE
{
	class DaeSolver_Parameters
	{
	public:

		enum DAE_INTEGRATOR { DAE_INTEGRATOR_OPENSMOKEPP, DAE_INTEGRATOR_BZZDAE, DAE_INTEGRATOR_IDA, DAE_INTEGRATOR_DASPK };
	...
	public:
		...
		*@param type integrator type: DAE_INTEGRATOR_OPENSMOKEPP, DAE_INTEGRATOR_BZZDAE, DAE_INTEGRATOR_IDA, DAE_INTEGRATOR_DASPK
		*/
		...
		*/
		void SetRelativeTolerance(const double relative_tolerance) { relative_tolerance_ = relative_tolerance; scalar_relative_tolerance_ = true; }

		/**
		*@brief Sets the absolute tolerance
		*@param absolute_tolerance the absolute tolerance (same for all the variables)
		*/
		void SetAbsoluteTolerance(const double absolute_tolerance) { absolute_tolerance_ = absolute_tolerance; scalar_absolute_tolerance_ = true; }

		/**
		*@brief Sets the relative tolerances
		*@param relative_tolerance the relative tolerances 
		*/
		void SetRelativeTolerances(const Eigen::VectorXd& relative_tolerances) { relative_tolerances_ = relative_tolerances;  scalar_relative_tolerance_ = false; }

		/**
		*@brief Sets the absolute tolerances
		*@param absolute_tolerance the absolute tolerances
		*/
		void SetAbsoluteTolerances(const Eigen::VectorXd& absolute_tolerances) { absolute_tolerances_ = absolute_tolerances;  scalar_absolute_tolerance_ = false; }

		/**
		*@brief Returns true if relative tolerances are the same for all the variables
		*/
		bool scalar_relative_tolerance() const { return scalar_relative_tolerance_;  }

		/**
		*@brief Returns true if absolute tolerances are the same for all the variables
		*/
		bool scalar_absolute_tolerance() const { return scalar_absolute_tolerance_; }
		...
		*/
		...
		/**
		*@brief Returns the integrator type: DAE_INTEGRATOR_OPENSMOKEPP, DAE_INTEGRATOR_BZZDAE, DAE_INTEGRATOR_IDA, DAE_INTEGRATOR_DASPK
		*/
		...
		*/
		...
		/**
		*@brief Returns the Jacobian matrix structure
		*/
		...
		*@brief Returns the relative tolerance
		*/
		const Eigen::VectorXd& relative_tolerances() const { return relative_tolerances_; }

		/**
		*@brief Returns the absolute tolerance
		*/
		const Eigen::VectorXd& absolute_tolerances() const { return absolute_tolerances_; }
	...
	private:
		...
		DAE_INTEGRATOR type_;									//!< DAE integrator type
		
		bool scalar_relative_tolerance_;						//!< scalar relative tolerance
		bool scalar_absolute_tolerance_;						//!< scalar absolute tolerance
		double relative_tolerance_;								//!< relative tolerance
		double absolute_tolerance_;								//!< absolute tolerance
		Eigen::VectorXd relative_tolerances_;					//!< relative tolerances
		Eigen::VectorXd absolute_tolerances_;					//!< absolute tolerances
		...
		OpenSMOKE::JacobianStructureType jacobian_structure_;	//!< Jacobian matrix structure
...
#include "DaeSolver_Parameters.hpp"

#endif	/* DaeSolver_Parameters_H */
