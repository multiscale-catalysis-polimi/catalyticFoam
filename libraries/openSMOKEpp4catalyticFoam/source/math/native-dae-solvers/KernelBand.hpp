...
namespace DaeSMOKE
{
	...
	template <typename DAESystemObject>
	bool KernelBand<DAESystemObject>::BuildAndFactorizeMatrixG(const double hr0, const double rr0)
	{	
	...
	template <typename DAESystemObject>
	bool KernelBand<DAESystemObject>::SolveLinearSystem(Eigen::VectorXd& db)
	{
		const double tstart = OpenSMOKE::OpenSMOKEGetCpuTime();

		const int ier = G_->Solve(db.data());
