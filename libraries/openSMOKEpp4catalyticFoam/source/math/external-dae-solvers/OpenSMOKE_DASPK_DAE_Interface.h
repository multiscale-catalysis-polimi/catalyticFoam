...
#define COMPLETE_DAESOLVERINTERFACE_DASPK_DAE(classname)\
...
#define DEFINE_DAESOLVERINTERFACE_DASPK_DAE(classname)\
private:\
	\
...
public:\
	...
	{\
		...
		for(unsigned int i=0;i<m_this->n_;++i) \
			if (m_this->algebraic_[i]==false) 	delta[i] = dy[i] - m_this->fy_[i]; \
			else								delta[i] = -m_this->fy_[i]; \
	}\
	\
