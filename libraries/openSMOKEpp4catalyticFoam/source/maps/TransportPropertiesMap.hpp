...
namespace OpenSMOKE
{
	...
	void TransportPropertiesMap::MassBinaryDiffusionCoefficients(double* gammaBin)
	{
		gamma();
		for(unsigned int i = 0; i < gammaSpecies_.size(); i++)
			gammaBin[i] = gammaSpecies_[i];
	}

    void TransportPropertiesMap::SpeciesViscosity(double* etaBin)
	{
		eta();
		for(unsigned int i = 0; i < etaSpecies_.size(); i++)
			etaBin[i] = etaSpecies_[i];
	}
